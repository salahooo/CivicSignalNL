...
export const createBasicAuthorization=(u:string,p:string)=>'Basic '+btoa(String.fromCharCode(...new TextEncoder().encode(`${u}:${p}`)));
export function AdminAuthProvider({children}:{children:ReactNode}){const[authorization,setAuthorization]=useState<string|null>(null);useEffect(()=>configureAdminClient(authorization,()=>setAuthorization(null)),[authorization]);const login=async(u:string,p:string)=>{const h=createBasicAuthorization(u,p);await adminMe(h);setAuthorization(h)};return <C.Provider value={{authorization,login,logout:()=>setAuthorization(null)}}>{children}</C.Provider>};export const useAdmin=()=>{const c=useContext(C);if(!c)throw new Error('AdminAuthProvider ontbreekt');return c};
