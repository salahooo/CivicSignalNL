...
export type AmsterdamPreview = { hasLocation?: boolean; reportId: string; category: string; district: string; occurredAt: string; sourceType: string; sourceName: string }
export type AmsterdamImportResult = { mapped?: number; withLocation?: number; withoutLocation?: number; confirmationToken?: string; fetched: number; published: number; skipped: number; failed: number; startedAt: string; completedAt: string; nextPageAvailable: boolean; previewItems?: AmsterdamPreview[] }
