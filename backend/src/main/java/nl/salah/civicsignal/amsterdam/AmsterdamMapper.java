...
public class AmsterdamMapper {
    public ReportEvent map(AmsterdamRecord record) {
        ...
        Instant completed=record.datumAfgerond()==null||record.tijdstipAfgerond()==null?null:LocalDate.parse(record.datumAfgerond()).atTime(LocalTime.parse(record.tijdstipAfgerond())).atZone(ZoneId.of("Europe/Amsterdam")).toInstant();Integer days=record.resolutionDays()!=null&&record.resolutionDays()>=0?record.resolutionDays():null;ReportLocation location=location(record.latitudeVisualisatie(),record.longitudeVisualisatie());
    ...
    private ReportLocation location(Double lat,Double lon){if(lat==null||lon==null||lat<50.7||lat>53.7||lon<3.2||lon>7.3)return null;return new ReportLocation(Math.round(lat*10000d)/10000d,Math.round(lon*10000d)/10000d);}
}
