...
public class AmsterdamAdapterService {
    ...
    private Integer number(JsonNode n,String key,Integer fallback){return n.path(key).isNumber()?n.path(key).asInt():fallback;} private Double decimal(JsonNode n,String key){return n.path(key).isNumber()?n.path(key).asDouble():null;}
