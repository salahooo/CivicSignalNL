...
public class AmsterdamAdapterService {
    ...
    private AmsterdamRecord record(JsonNode n) { return new AmsterdamRecord(text(n,"id"),text(n,"hoofdcategorie"),text(n,"subcategorie"),text(n,"externeStatus"),text(n,"datumMelding"),text(n,"tijdstipMelding"),text(n,"gbdStadsdeelNaam"),text(n,"gbdWijkNaam"),text(n,"gbdBuurtNaam"),text(n,"laatstGezienBron"),text(n,"bagWoonplaatsNaam"),text(n,"datumAfgerond"),text(n,"tijdstipAfgerond"),number(n,"werkelijkeDoorlooptijdDagen",number(n,"doorlooptijdDagen",null)),decimal(n,"latitudeVisualisatie"),decimal(n,"longitudeVisualisatie")); }
    private String text(JsonNode n,String key) { return n.path(key).isMissingNode()||n.path(key).isNull()?null:n.path(key).asText(); }
    private Integer number(JsonNode n,String key,Integer fallback){return n.path(key).isNumber()?Integer.valueOf(n.path(key).asInt()):fallback;} private Double decimal(JsonNode n,String key){return n.path(key).isNumber()?Double.valueOf(n.path(key).asDouble()):null;}
